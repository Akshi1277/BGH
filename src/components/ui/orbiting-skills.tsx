...
// --- Type Definitions ---
type IconType = 'html' | 'css' | 'javascript' | 'react' | 'node' | 'tailwind' | 'python' | 'aws';
...
const iconComponents: Record<IconType, { component: () => React.JSX.Element; color: string }> = {
  ...
  python: {
    component: () => (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-full h-full">
        <path d="M11.898 0c-5.26 0-4.943 2.28-4.943 2.28l.006 2.361h4.996v.707H4.991s-3.488.39-3.488 4.962c0 4.57 3.033 4.415 3.033 4.415h1.811v-2.544s-.096-3.034 2.986-3.034h5.111s2.84.048 2.84-2.744V2.28S17.472 0 11.898 0zm-2.73 1.393c.477 0 .864.387.864.864 0 .477-.387.864-.864.864-.477 0-.864-.387-.864-.864 0-.477.387-.864.864-.864zm2.748 22.607c5.26 0 4.943-2.28 4.943-2.28l-.006-2.361h-4.996v-.707h6.966s3.488-.39 3.488-4.962c0-4.57-3.033-4.415-3.033-4.415h-1.811v2.544s.096 3.034-2.986 3.034h-5.111s-2.84-.048-2.84 2.744v4.137s-.188 2.28 5.386 2.28zm2.73-1.393c-.477 0-.864-.387-.864-.864 0-.477.387-.864.864-.864.477 0 .864.387.864.864 0 .477-.387.864-.864.864z" fill="#3776AB"/>
      </svg>
    ),
    color: '#3776AB'
  },
  aws: {
    component: () => (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-full h-full">
        <path d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0zm5.12 16.5c-2.14 1.57-5.06 2.15-7.72 1.41-3.47-.96-6.18-3.9-6.73-7.46-.66-4.29 2.1-8.35 6.36-9.28 4.41-.96 8.76 1.76 9.87 6.17.15.59.22 1.19.22 1.79 0 1.25-.33 2.47-.96 3.55-.42.72-1.04 1.34-1.04 1.34s.88-.13 1.48-.38c.6-.25 1.1-.6 1.1-.6s-1.12 1.83-2.58 3.46z" fill="#FF9900"/>
      </svg>
    ),
    color: '#FF9900'
  }
...
const OrbitingSkill = memo(({ config, angle }: OrbitingSkillProps) => {
  ...
  return (
    ...
    >
      <div
        className={`
          relative w-full h-full p-2 bg-[#08101E]/95 border border-[#38BDF8]/30 backdrop-blur-md
          rounded-full flex items-center justify-center
          transition-all duration-300 cursor-pointer
          ${isHovered ? 'scale-125 shadow-2xl border-[#38BDF8]' : 'shadow-lg hover:shadow-xl'}
        `}
        style={{
          boxShadow: isHovered
      ...
      >
        ...
        {isHovered && (
          <div className="absolute -bottom-9 left-1/2 -translate-x-1/2 px-2.5 py-1 bg-[#04070D]/95 border border-[#38BDF8]/40 shadow-xl backdrop-blur-md rounded-md text-[11px] font-mono text-[#38BDF8] font-bold whitespace-nowrap pointer-events-none z-30">
            {label}
          </div>
        )}
      </div>
  ...
  return (
    <div className="w-full flex items-center justify-center overflow-hidden py-6">
      ...
      >
        ...
        {/* Central Code Icon */}
        <div className="w-16 h-16 sm:w-20 sm:h-20 bg-gradient-to-br from-[#0F172A] to-[#04070D] border border-[#38BDF8]/40 rounded-full flex items-center justify-center z-10 relative shadow-[0_0_30px_rgba(56,189,248,0.3)]">
          <div className="absolute inset-0 rounded-full bg-[#38BDF8]/20 blur-xl animate-pulse"></div>
          ...
          <div className="relative z-10 text-[#38BDF8]">
            <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="url(#cyan-purple-grad)" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
              <defs>
