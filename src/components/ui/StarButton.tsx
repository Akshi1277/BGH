...
export function StarButton({
  ...
  const contentMarkup = (
    <>
      {/* 1. Light Beam Layer (z-[1]) - Rotates around the border path */}
      <div
        className="absolute aspect-square inset-0 animate-star-btn bg-[radial-gradient(ellipse_at_center,var(--light-color),transparent,transparent)] pointer-events-none z-[1]"
        style={
          {
            offsetPath: "var(--path)",
            offsetDistance: "0%",
      ...
      {/* 2. Star Background Mask AND Border Container (z-[4]) */}
      <div
        className="absolute inset-0 border border-solid border-white/20 z-[4] overflow-hidden rounded-[inherit] pointer-events-none"
        style={{ borderWidth: "var(--border-width)" }}
        aria-hidden="true"
      >
        {/* The SVG is filled with black, which masks the light beam everywhere EXCEPT the 1.5px border margin and the transparent star holes! */}
        <StarBackground color={backgroundColor} />
      </div>

      {/* 3. Text Label */}
      <span className="z-10 relative bg-gradient-to-t from-white via-white to-neutral-400 inline-flex items-center gap-2 text-transparent bg-clip-text font-medium select-none">
        {children}
        {icon && <span className="inline-flex items-center text-white">{icon}</span>}
      </span>
    </>
  );

  const sharedClassName = cn(
    "relative z-[3] overflow-hidden h-11 px-6 py-2.5 inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-3xl text-sm font-medium transition-transform hover:scale-105 active:scale-95 disabled:pointer-events-none disabled:opacity-50 group/star-button bg-black", // Removed the border class from here!
    className
  );
