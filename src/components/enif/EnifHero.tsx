...
export default function EnifHero() {
  ...
  useEffect(() => {
    ...
    // Pause video when out of viewport to save CPU/GPU
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            videoRef.current?.play().catch(() => {
              // Ignore auto-play restriction errors if they happen
            });
          } else {
  ...
  return (
    <section className="relative overflow-hidden bg-[#04070D] text-[#F8FAFC] pt-24 pb-12 sm:pt-32 sm:pb-20 md:pt-40 md:pb-28 min-h-[100svh] min-h-screen flex flex-col justify-center border-b border-[#1E293B]">
      ...
      <div className="absolute inset-0 z-0 opacity-70 pointer-events-none flex items-center justify-center overflow-hidden">
        <video
          ...
          onLoadedData={() => setVideoLoaded(true)} // Fallback for some browsers
